    ...
    protected final void runInstructionLogic(final CPU currentCpu, final int[] data) {
        switch (getAddressMode()) {
            ...
            case DATA_8_BIT_TO_REGISTER:
                this.substractDirectDataToRegister(currentCpu, data);
                break;
            default:
                throw new IllegalArgumentException(
                        "Address mode not supported for SUB instruction: " + getAddressMode());
        }
    }

    private void substractDirectDataToRegister(final CPU currentCpu, final int[] data) {
